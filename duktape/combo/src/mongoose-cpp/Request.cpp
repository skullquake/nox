...
namespace Mongoose{
	...
	void Request::setData(std::string a){
		this->data=a;
	}
	void Request::setMethod(std::string a){
		this->method=a;
	}
	void Request::setUrl(std::string a){
		this->url=a;
	}
	...
	void Request::handleUploads(){
		...
		const char *data;
		int data_len;
		if(mg_parse_multipart(connection->content,connection->content_len,var_name,sizeof(var_name),file_name,sizeof(file_name),&data,&data_len)){
